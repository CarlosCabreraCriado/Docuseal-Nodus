<template>
  <div id="form_completed" class="mx-auto max-w-md flex flex-col" dir="auto">
        ...
        {{
          ...
          (hasSignatureFields
            ? hasMultipleDocuments
              ? t("documents_have_been_signed")
              : t("document_has_been_signed")
            : t("form_has_been_completed"))
        }}
      </span>
    </div>
    <div v-if="completedMessage.body" class="mt-2">
    ...
    <div v-if="attribution" class="text-center mt-4">
      {{ t("powered_by") }}
      <a href="#" target="_blank" class="underline">NodusDevelopment</a> -
...
export default {
  ...
  methods: {
    ...
    download() {
      ...
      fetch(this.baseUrl + `/submitters/${this.submitterSlug}/download`).then(
        async (response) => {
          if (response.ok) {
            const urls = await response.json();
            const isMobileSafariIos =
